...
class RAGPipeline:
    ...
    def __init__(self, search_engine: SearchEngine, llm_client):
        ...
        self.SYSTEM_PROMPT = {
            ...
            2.  **【重要】信息来源层级**: 
                * 【原始上下文】是回答问题的唯一合法来源。
                * 【对话历史】仅用于理解用户问题的指代关系（例如 "它" 指的是什么），绝不能作为回答问题的信息来源。
            3.  **信息不足则明确告知**: 如果【原始上下文】中的信息不足以回答【问题】（即使你在对话历史中见过答案），你必须明确回答：“根据我所掌握的文档资料，无法回答您的问题。”
            4.  **极致纯净的答案**: 你的回答必须只直接回答用户的问题核心。绝对禁止输出“参考文档”等类似信息，这些将由程序在外部自动添加。
            5.  **保留来源**: 你需要在最终答案的末尾，附上信息来源的文件名。
            """
        }
